<template>
    <header class="sticky top-0 z-50 transition-all duration-300">
     <section :class="[w-full, isScrolled ? 
     'bg-gray-100/95 backdrop blur shadow-md py-2':'bg-gray-100 py-4']">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div 
                ...
                <!--Search Bar-->
                 <form class="w-full md:flex-1 max-w-sm" role="search"
                  aria-label="Site search">
                   <label class="relative w-full">
                      <input type="text" placeholder="Search ..."
                      class="w-full px-3 py-2 text-xs border border-gray-300 
                      rounded-full focus:outline none 
                      focus:ring-2 focus:ring-indigo-500">

                      <button type="submit"
                      class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500
                       hover:text-indigo600" aria-label="Search button"
                      >
                 ...
                 <!--Icons-->
                  <aside class="flex items-center justify-end space-x-4 w-full md:w-auto">
                   ...
                   aria-label="Wishlist">
                       ...
                       <span
                       class="absolute -top-1 -right-1 bg-pink-500 text-white text-xs 
                       rounded-full h-5 w-5 flex items-center justify-center">3</span>
                   </button>
                   <button 
                   @click="isCartOpen = !isCartOpen" class="relative p-2 text-gray-700 
                   hover:text-pink-600" aria-label="Cart">
                   <Icon icon="mdi:cart-outline" class="w-5 h-5" />
                   <span class="absolute -top-1 -right-1 bg-pink-500 text-white text-xs
                    rounded-full h-5 w-5 flex items-center justify-center">{{ cart.cartCount }}
                   </span>
                  </button>
                   <button class="relative p-2 text-gray-700 hover:text-pink-600" 
                   aria-label="Wishlist">
        ...
        <!--Navbar Links-->
         <nav class="bg-pink-950" aria-label="Main navigation">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <ul class="hidden md:flex justify-center py-3 flex-wrap gap-x-6 text-sm 
                ...
                class="md:hidden mt-2 bg-white rounded-lg shadow-md p-4 space-y-3 
                text-[#5D4037] text-center"
