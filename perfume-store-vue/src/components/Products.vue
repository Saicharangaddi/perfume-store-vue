    ...
    <section class="py-12 scroll-mt-20 bg-gray-50" id="products">
        <main class="container mx-auto px-4">
            <header class="flex justify-between items-center mb-8">
                <h2 class="md:txt-3xl text-2xl font-bold text-amber-900">Featured Products</h2>
            ...
            <section class="relative overflow-hidden">
                <ul class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <li 
                    v-for="product in visibleProducts" :key="product.id"
                ...
                <article>
                    ...
                    <figcaption v-if="product.discount > 0"
                        class="absolute top-3 right-3 bg-red-500 text-white text-xs font-bold px-2 py-1 
                        rounded-full">
                        -{{ product.discount }}%
                    </figcaption>

                    <button
                    class="absolute top-3 left-3 p-2 bg-white rounded-full shadow-md hover:bg-pink-100 
                    ...
                    <section class="p-4">
                        <div class="flex items-center mb-2">
                            ...
                            <span class="text-sm text-gray-500  ml-1">({{ product.rating.rate }})</span>
                        </div>
...
const fetchProducts = async () => {
  try {
    const response = await axios.get('/perfumes.json'); // Example API
