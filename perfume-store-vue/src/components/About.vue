      ...
      <div class="flex flex-col lg:flex-row gap-12 mb-20">
        ...
        <div class="lg:w-1/2">
          <div class="relative rounded-2xl overflow-hidden h-96 shadow-xl">
            <img :src="product6" alt="Luxury Perfumes" class="w-full h-full object-cover" />
        ...
        <div class="lg:w-1/2 flex flex-col justify-center">
          <h2 class="text-4xl font-bold text-amber-900 mb-6">{{ aboutData?.content.title }}</h2>
          <p class="text-lg text-gray-600 mb-4">{{ aboutData?.content.description }}</p>
...
onMounted(async () => {
  try {
    const response = await axios.get('/aboutData.json') 
